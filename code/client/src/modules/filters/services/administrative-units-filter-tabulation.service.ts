...
const LOG_PREFIX: string = "[Administrative Units Filter Tabulation Service]";


@Injectable({ providedIn: 'root' })
export class AdministrativeUnitsFilterTabulationService implements OnDestroy {
        ...
        private log: NGXLogger) {
            this._subscriptions.push(
                this.filterService.filter$
                    .subscribe(
                        (filter: Filter) => {
                            this._transform(filter.accountabilities);
                        }));
    ...
    private _set(patch: Partial<State>) {
        ...
        this._transform(this.filterService.currentFilter.accountabilities);
    ...
    _transform(records: Accountability[]) {
        ...
        if (records.length != 0) {
            ...
            let transformed: Accountability[] = records.filter(contextIssueIndicator => this.matches(contextIssueIndicator, searchTerm));
