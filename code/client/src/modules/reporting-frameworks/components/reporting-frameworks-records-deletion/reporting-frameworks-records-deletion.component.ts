...
export class ReportingFrameworksRecordsDeletionComponent implements OnInit, AfterContentInit {
  ...
  ngOnInit() {
    ...
    // Retrieve the ReportingFramework with the given id from the data store 
    this.log.trace(`${LOG_PREFIX} Retrieving the ReportingFramework with the given id from the data store`);
  ...
  /**
   * Deletes Reporting Framework records.
    ...
    this.reportingFrameworksDataService
      ...
      .subscribe(
        (response: number) => {
          ...
          // The Reporting Framework record was deleted successfully
