...
const LOG_PREFIX: string = "[Emissions Types Records Deletion Component]";

@Component({
  selector: 'sb-emissions-types-records-deletion',
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './emissions-types-records-deletion.component.html',
  styleUrls: ['emissions-types-records-deletion.component.scss'],
})
export class EmissionsTypesRecordsDeletionComponent implements OnInit, AfterContentInit {
  ...
  constructor(private emissionsTypesDataService: EmissionsTypesDataService, private log: NGXLogger) { }

  ngOnInit() {

    this.log.trace(`${LOG_PREFIX} Initializing Component`);

    // Retrieve the EmissionType with the given id from the data store 
    this.log.trace(`${LOG_PREFIX} Retrieving the EmissionType with the given id from the data store`);
    this.log.debug(`${LOG_PREFIX} Emission Type record Id = ${this.id}`);
    this.emissionType = this.emissionsTypesDataService.records.find(d => d.id == this.id);
    ...
    this.emissionsTypesDataService
      ...
      .subscribe(
        (response: number) => {
          ...
          // The Emission Type record was deleted successfully
