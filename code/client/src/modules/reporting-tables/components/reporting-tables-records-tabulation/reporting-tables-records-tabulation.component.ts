...
import { ReportingTablesRecordsTabulationService } from '@modules/reporting-tables/services/reporting-tables-records-tabulation.service';
import { ReportingTablesRecordsCreationModalComponent, ReportingTablesRecordsUpdationModalComponent, ReportingTablesRecordsDeletionModalComponent } from '@modules/reporting-tables/containers';

const LOG_PREFIX: string = "[Reporting Tables Records Tabulation Component]";

@Component({
    selector: 'sb-reporting-tables-records-tabulation',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './reporting-tables-records-tabulation.component.html',
    styleUrls: ['reporting-tables-records-tabulation.component.scss'],
})
export class ReportingTablesRecordsTabulationComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    //Keep tabs on the target reportingTable category
    ...
    constructor(
        public reportingTablesTableService: ReportingTablesRecordsTabulationService,
    ...
    ngAfterViewInit() {
        ...
        this.pagination.initialize(this.reportingTablesTableService.page, this.pageSize);
        ...
        this._subscriptions.push(
            this.reportingTablesTableService.total$.subscribe(
                (total) => {
        ...
        this._subscriptions.push(
            this.reportingTablesTableService.loading$.subscribe(
                (loading) => {
    ...
    onSearch(event: any) {
        ...
        this.reportingTablesTableService.searchTerm = event;
        this.cd.detectChanges();
    }
    ...
    onSort({ column, direction }: SortEvent) {
        ...
        this.reportingTablesTableService.sortColumn = column;
        this.reportingTablesTableService.sortDirection = direction;
        this.cd.detectChanges();
    }
    ...
    onPageChange(event: any) {
        ...
        this.reportingTablesTableService.page = event;
        this.cd.detectChanges();
    }
    ...
    onPageSizeChange(event: any) {
        ...
        this.reportingTablesTableService.pageSize = event;
        this.cd.detectChanges();
    }

    /**
     * Propagates reportingTable category id change events to the table service
     * @param event The newly desired reportingTable category id
     */
    onReportingFrameworkChange(event: any) {
        this.log.trace(`${LOG_PREFIX} Changing Reporting Framework Id to ${event}`);
        this.reportingTablesTableService.reportingFrameworkId = event;
        this.cd.detectChanges();
    }

    /**
     * Propagates Reporting Tables records Addition Requests to the responsible component
     */
    onAddReportingTable() {
        this.log.trace(`${LOG_PREFIX} Adding a new Reporting Table record`);
        this.log.debug(`${LOG_PREFIX} Reporting Table Category = ${JSON.stringify(this.targetReportingFramework)}`);
        const modalRef = this.modalService.open(ReportingTablesRecordsCreationModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.targetReportingFramework = this.targetReportingFramework;
    }

    /**
     * Propagates Reporting Tables records Updation Requests to the responsible component
     */
    onUpdateReportingTable(id: number) {
        this.log.trace(`${LOG_PREFIX} Updating Reporting Table record`);
        this.log.debug(`${LOG_PREFIX} Reporting Table record Id = ${id}`);
        this.log.debug(`${LOG_PREFIX} Reporting Table Category = ${JSON.stringify(this.targetReportingFramework)}`);
        const modalRef = this.modalService.open(ReportingTablesRecordsUpdationModalComponent, { centered: true, backdrop: 'static' });
    ...
    /**
     * Propagates Reporting Tables records Deletion Requests to the responsible component
     */
    onDeleteReportingTable(id: number) {
        this.log.trace(`${LOG_PREFIX} Deleting Reporting Table record`);
        this.log.debug(`${LOG_PREFIX} Reporting Table record Id = ${id}`);
        this.log.debug(`${LOG_PREFIX} Reporting Table Category = ${JSON.stringify(this.targetReportingFramework)}`);
        const modalRef = this.modalService.open(ReportingTablesRecordsDeletionModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;
        modalRef.componentInstance.targetReportingFramework = this.targetReportingFramework;
    }
