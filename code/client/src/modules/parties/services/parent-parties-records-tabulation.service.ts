...
const LOG_PREFIX: string = "[Parent Parties Records Tabulation Service]";

export interface ParentPartyState extends State {
    partyTypeId: number | null;
}

@Injectable({ providedIn: 'root' })
export class ParentPartiesRecordsTabulationService implements OnDestroy {

    // The user defined search or sort criteria.
    private _state: ParentPartyState = { partyTypeId: null, page: 1, pageSize: 4, searchTerm: '', sortColumn: '', sortDirection: '' };
     ...
     */
    set(patch: Partial<ParentPartyState>) {
