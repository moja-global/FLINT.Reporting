...
const LOG_PREFIX: string = "[Subsidiary Parties Records Tabulation Service]";

export interface SubsidiaryPartyState extends State {
    partyTypeId: number | null;
}

@Injectable({ providedIn: 'root' })
export class SubsidiaryPartiesRecordsTabulationService implements OnDestroy {

    // The user defined search or sort criteria.
    private _state: SubsidiaryPartyState = { partyTypeId: null, page: 1, pageSize: 4, searchTerm: '', sortColumn: '', sortDirection: '' };
     ...
     */
    set(patch: Partial<SubsidiaryPartyState>) {
