...
# -------------------------------------------------------------------------------------
if [ $TASKS -eq 1 ]; then
	bash $PROJECT_DIR/services/tasks/register.sh
fi
...
if [ $TASK_MANAGER -eq 1 ]; then
	bash $PROJECT_DIR/services/task-manager/register.sh
fi
...
if [ $DATA_PROCESSOR -eq 1 ]; then
	bash $PROJECT_DIR/services/data-processor/register.sh
fi
...
if [ $DATA_AGGREGATOR -eq 1 ]; then
	bash $PROJECT_DIR/services/data-aggregator/register.sh
fi
...
if [ $CRF_TABLES -eq 1 ]; then
	bash $PROJECT_DIR/services/crf-tables/register.sh
fi
