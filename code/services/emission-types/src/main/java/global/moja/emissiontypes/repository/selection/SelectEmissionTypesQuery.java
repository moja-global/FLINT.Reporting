...
@Component
@Slf4j
public class SelectEmissionTypesQuery {

    @Autowired
    DatabaseConfig databaseConfig;

    /**
     * Selects all or specific Emission Types records from the database depending on whether or not
     * query parameters were supplied as part of the query
     *
     * @return a list of Emission Types records if found
     */
    public Flux<EmissionType> selectEmissionTypes(QueryParameters parameters) {

        log.trace("Entering selectEmissionTypes()");

        String query =
                "SELECT * FROM emission_type" +
