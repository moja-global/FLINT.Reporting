...
public class UpdateEmissionTypesQuery {
	...
	/**
	 * Recursively Updates emission type records
	...
	public Flux<Integer> updateEmissionTypes(EmissionType[] emissionTypes) {
		...
		String query = "UPDATE emission_type SET name = ?, description = ? WHERE id = ?";
	...
	private Flowable getParametersListStream(EmissionType[] emissionTypes) {
		...
		for (EmissionType p : emissionTypes) {
			parameters.add(Arrays.asList(p.getName(), p.getAbbreviation(), p.getDescription(), p.getId()));
		}
