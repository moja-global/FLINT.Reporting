...
@ContextConfiguration(initializers = CreatePartyTypeIT.Initializer.class)
public class CreatePartyTypeIT {
    ...
    @Test
    public void Given_PartyTypeDetails_When_Post_Then_PartyTypeRecordWillBeCreatedAndReturned() {

        webTestClient
                .post()
                .uri("/api/v1/party_types")
                .contentType(MediaType.APPLICATION_JSON)
                .body(Mono.just(partyType4), PartyType.class)
                .exchange()
                .expectStatus().isCreated()
                .expectBody(PartyType.class)
                .value(response -> {
                            Assertions.assertThat(response.getId()).isEqualTo(4L);
                            Assertions.assertThat(response.getParentPartyTypeId()).isEqualTo(partyType4.getParentPartyTypeId());
                            Assertions.assertThat(response.getName()).isEqualTo(partyType4.getName());
