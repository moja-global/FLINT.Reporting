...
@Component
@Slf4j
public class DeleteLandUseCategoryHandler {

	@Autowired
    LandUseCategoriesRepository repository;
	
	/**
	 * Deletes a Land Use Category record
	 *
	 * @param request the request containing the details of the Land Use Category record to be deleted
	...
	public Mono<ServerResponse> deleteLandUseCategory(ServerRequest request) {
		...
		return 
			ServerResponse
				...
				.body(deleteLandUseCategoryById(Long.parseLong(request.pathVariable("id"))),Integer.class)
				.onErrorMap(e -> new ServerException("Land Use Category deletion failed", e));

	}
	
	private Mono<Integer> deleteLandUseCategoryById(Long id){
		
		return 
			repository
				.deleteLandUseCategoryById(id);
	}
