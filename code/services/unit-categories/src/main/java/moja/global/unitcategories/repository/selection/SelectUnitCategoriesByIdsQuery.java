...
@Component
@Slf4j
public class SelectUnitCategoriesByIdsQuery {

	@Autowired
    DatabaseConfig databaseConfig;

	/**
	 * Selects specific unit category records from the database given their unique identified
	 * @param ids the unique identifiers of the records
	 * @return a list of unit category records corresponding to the identifiers if found
	 */
	public Flux<UnitCategory> selectUnitCategoriesByIds(Long[] ids) {

		String query = "SELECT * FROM unit_category WHERE id in (?)";
