...
public class RetrieveReportingTableHandler {
	...
	/**
	 * Retrieves a Reporting Table record given its unique identifier
	 *
	...
	public Mono<ServerResponse> retrieveReportingTable(ServerRequest request) {
		...
		return
			ServerResponse
				...
				.body(retrieveReportingTableById(Long.parseLong(request.pathVariable("id"))),
                        ReportingTable.class)
				.onErrorMap(e -> new ServerException("Reporting Table retrieval failed", e));
	}
