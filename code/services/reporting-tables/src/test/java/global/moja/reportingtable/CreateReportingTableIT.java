...
public class CreateReportingTableIT {
    ...
    public void Given_ReportingTableDetails_When_Post_Then_ReportingTableRecordWillBeCreatedAndReturned() {
        ...
        webTestClient
                ...
                .body(Mono.just(reportingTable4), ReportingTable.class)
                .exchange()
                .expectStatus().isCreated()
                .expectBody(ReportingTable.class)
                .value(response -> {
                            Assertions.assertThat(response.getId()).isEqualTo(4L);
                            Assertions.assertThat(response.getReportingFrameworkId()).isEqualTo(reportingTable4.getReportingFrameworkId());
                            Assertions.assertThat(response.getNumber()).isEqualTo(reportingTable4.getNumber());
                            Assertions.assertThat(response.getName()).isEqualTo(reportingTable4.getName());
                            Assertions.assertThat(response.getDescription()).isEqualTo(reportingTable4.getDescription());
